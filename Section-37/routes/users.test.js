...
describe("POST /users/:username/jobs/:jobID", function () {
  ...
  test("bad request with invalid user", async function () {
    const resp = await request(app)
      .post("/users/u4/jobs/3")
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
  });

  test("bad request with invalid job", async function () {
    const resp = await request(app)
...
describe("GET /users", function () {
  test("works for admins", async function () {
    const resp = await request(app)
      .get("/users")
      .set("authorization", `Bearer ${a1Token}`);
    expect(resp.body.users).toContainEqual(
      {
        username: "u1",
        firstName: "U1F",
        lastName: "U1L",
...
describe("GET /users/:username", function () {
  ...
  test("works for specified user", async function () {
    const resp = await request(app)
      .get(`/users/u1`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({
...
describe("DELETE /users/:username", function () {
  ...
  test("unauth for non-admins and non-user", async function () {
    const resp = await request(app)
      .delete(`/users/u2`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
